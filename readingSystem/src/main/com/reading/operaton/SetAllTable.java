...
//				System.out.println(rs1.getString(1).trim());
				model.addRow(new Object[] { ++i, rs1.getString(1).trim(), rs1.getString(2).trim(),
						rs1.getString(8).trim(), rs1.getString(7).trim(), rs1.getString(6).trim(),
						rs1.getString(11).trim(), rs1.getString(5).trim(), rs1.getString(3).trim(),
						rs1.getString(14).trim(), rs1.getString(13).trim(),
						new FindStatus(totlFeesHm.get(rs1.getString(1).trim()), rs1.getString(12).trim(),
								rs1.getString(14).trim(), getFees(rs1.getString(12).trim(), rs1.getString(13).trim()))
										.getStatus() });
			}
			rs1.close();
		} catch (Exception e) {
	...
	public void setClosedAdmissionsTable(JTable table) {
		try {
			...
			while (rs.next()) {
				model.addRow(new Object[] { ++i, rs.getString(1).trim(), rs.getString(2).trim(), rs.getString(3).trim(),
						rs.getString(8).trim(), rs.getString(9).trim(), rs.getString(10).trim(), rs.getString(6).trim(),
						rs.getString(10).trim(), rs.getString(14).trim(), rs.getString(12).trim(),
...
//				System.out.println(rs.getString(2).trim());
	...
	public void setStudentToFields(String studentId, ShowStudentDetails ssd) {
		...
		try {
			...
			new ProcessImage().getImage(rs.getString(1).trim(), ssd.updateImageLbl);// set image
		} catch (Exception e) {
			e.printStackTrace();
		}
	}
